...
import { get_Added_Thick_Shake } from "../../Redux/Get_All_MenuItems/action";
import { delete_Added_Thick_Shake } from "../../Redux/Delete_All_MenuItem/action";
...
const CultureCrust = () => {
  ...
  const { loading, error, theek_shake } = useSelector(
  ...
  const handleDelete = (id) => {
    dispatch(delete_Added_Thick_Shake(id));
  };
  ...
  const pageCount = Math.ceil(theek_shake.length / itemsPerPage);
  const currentPageData = theek_shake.slice(offset, offset + itemsPerPage);
    ...
    <Layout>
      <Box>
        ...
        <Flex my={6} justifyContent={"space-between"}>
          ...
          <Button
            ...
            variant={"simpleWhite"}
          >
            <Link to="/add/thikshakes" style={linkStyle}>
              Add New Thick Shake
            </Link>
          </Button>
        </Flex>
      ...
      <PaginationBox>
        <ReactPaginate
          previousLabel={"<"}
          nextLabel={">"}
          pageCount={Math.ceil(theek_shake.length / itemsPerPage)}
