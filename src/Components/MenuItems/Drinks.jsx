...
const Drinks = () => {
  ...
  const pageCount = Math.ceil(drinks_can_pop.length / itemsPerPage);
  const currentPageData = drinks_can_pop.slice(offset, offset + itemsPerPage);
    ...
    <Layout>
              ...
              currentPageData.map((el, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <Center>
                      <Flex gap={8}>
                        <Link to={`/edit/drinkscan/${el.pizzaId}`}>
      ...
      <PaginationBox>
        <ReactPaginate
          previousLabel={"<"}
          nextLabel={">"}
          pageCount={Math.ceil(drinks_can_pop.length / itemsPerPage)}
