...
import { get_Added_Nanza } from "../../Redux/Get_All_MenuItems/action";
import { deleteOutline, edit, updown } from "../../assets";
import { linkStyle } from "../../data";
import CustomeFoodItes from "../CustomeFoodItes";
import Layout from "../Layout/Layout";
import { delete_Added_Nanza } from "../../Redux/Delete_All_MenuItem/action";
...
const Nanza = () => {
  ...
  const { loading, error, nanza } = useSelector(
  ...
  const handleDelete = (id) => {
    dispatch(delete_Added_Nanza(id));
  };
  ...
  const pageCount = Math.ceil(nanza.length / itemsPerPage);
  const currentPageData = nanza.slice(offset, offset + itemsPerPage);
    ...
    <Layout>
      ...
      <PaginationBox>
        <ReactPaginate
          previousLabel={"<"}
          nextLabel={">"}
          pageCount={Math.ceil(nanza.length / itemsPerPage)}
